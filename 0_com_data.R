...
write.csv(sites, file = '../data/processed/all_sites.csv')
write.csv(community, 
          file = '../data/processed/all_occurrences.csv')
...
subset_continent_taxa <- function(site_data, com_data, continent = 'America', order = 'Anura', write = TRUE) {
  ...
  if(write) {
    write.csv(site_data, 
              file = paste0('../data/processed/', continent, '_', order, '_site.csv'))  # Save site data
    write.csv(com_data, 
              file = paste0('../data/processed/', continent, '_', order, '_community.csv'))  # Save community data
  } else {
